...
## `chaotic-aur`
The first time when I used Athena OS is the OS itself has a repository called `chaotic-aur`, which is an _unofficial package repository_ that contains pre-built packages from the AUR. For example, you can install Discord, Zoom, Anydesk, or others through `chaotic-aur`.
...
Fortunately, I stumbled across the blog and found the fix, which is from the [kernel.org forum](https://bugzilla.kernel.org/show_bug.cgi?id=207423#c94). I have uploaded the bash file to fix the audio problem, which you can access it [here](./audio/RB14_2023_enable_internal_speakers_ver2.sh)
